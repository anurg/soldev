...
describe('escrow', () => {
  ...
  let [escrowPDA, escrowBump] = anchor.web3.PublicKey.findProgramAddressSync(
    [Buffer.from('escrow'), seed.toArrayLike(Buffer, 'le', 8), payer.publicKey.toBuffer()],
    program.programId,
  )
  ...
  const airdrop = async (key: anchor.web3.PublicKey) => {
    const tx = await provider.connection.requestAirdrop(key, 10 * anchor.web3.LAMPORTS_PER_SOL)
    await provider.connection.confirmTransaction(tx)
  }

  before(async () => {
    // Mint mint_a and Transfer to payer_ata_a 20000 tokens
    mint_a = await createMint(provider.connection, payer, payer.publicKey, null, 6)
    console.log(`mint_a-${mint_a}`)
    payer_ata_a = await getAssociatedTokenAddress(mint_a, payer.publicKey, true)
    console.log(`payer_ata_a-${payer_ata_a}`)
    const ataTx = await getOrCreateAssociatedTokenAccount(provider.connection, payer, mint_a, payer.publicKey, true)
    console.log(`ataTx-${ataTx}`)
    const mintTx = await mintTo(provider.connection, payer, mint_a, payer_ata_a, payer, 20000 * decimals)
    console.log(`mintTx-${mintTx}`)
    // Mint mint_b and Transfer to user_ata_b 10000 tokens
    await airdrop(user.publicKey)
    const balance = await provider.connection.getBalance(user.publicKey)
    console.log(`user balance- ${balance}`)
    mint_b = await createMint(provider.connection, user, user.publicKey, null, 6)
    ...
    const ataTxUser = await getOrCreateAssociatedTokenAccount(provider.connection, user, mint_b, user.publicKey, true)
    console.log(`ataTx-${ataTx}`)
    const mintTxUser = await mintTo(provider.connection, user, mint_b, user_ata_b, user, 20000 * decimals)
    ...
    let vaultTx = await getOrCreateAssociatedTokenAccount(provider.connection, payer, mint_a, escrowPDA, true)
    console.log(`vault - ${vaultTx}`)
    // Create maker_ata_b  ATA
    payer_ata_b = await getAssociatedTokenAddress(mint_b, payer.publicKey, true)
    ...
    let user_ata_aTx = await getOrCreateAssociatedTokenAccount(provider.connection, payer, mint_a, user.publicKey, true)
